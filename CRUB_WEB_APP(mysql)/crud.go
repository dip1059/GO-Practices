...
func Update(user User) bool {
	...
	if user.Email.String == "" {
		update, err = db.Query("UPDATE users SET full_name=?, email=NULL WHERE id=?;", user.Name.String, user.ID)
	} else {
		update, err = db.Query("UPDATE users SET full_name=?, email=? WHERE id=?;", user.Name.String, user.Email.String, user.ID)
	}
